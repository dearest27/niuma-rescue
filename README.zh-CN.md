...
- 确认后自动创建或复用独立 worktree，也支持 inline 模式直接在当前分支开发。
- 自动运行测试或 lint 验收门。
...
大仓库可以在 workspace 里配置 `work_mode: "inline"`，让 agent 直接在现有工作区和当前分支上修改。inline 模式不会创建 git worktree，不会 commit、push 或创建 PR/MR，最终由你人工决定怎么提交。它的 Review diff 会包含当前未提交文件；如果想让审查范围精确，开始前最好保持目标工作区干净。
